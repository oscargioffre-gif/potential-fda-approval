# FDA RADAR v13 — Verdetti sempre visibili + Storico in expander
...
def get_stock(ticker):
    try:
        ...
        vol_trend="neutro"
        if len(hist)>=10:
            rv=hist["Volume"].tail(5).mean(); ov=hist["Volume"].tail(15).head(10).mean()
            if ov>0:
                vc=(rv-ov)/ov
        ...
        pt="neutro"
        if len(hist)>=5:
            p5=hist["Close"].tail(5).mean(); pa=hist["Close"].mean()
            if pa>0:
                r=(p5-pa)/pa
                if r>0.05: pt="sale"
                elif r<-0.05: pt="scende"
        return {"price":round(price,2),"change":change,"cap":cap,"cap_str":fmt_cap(cap),"vol_now":vol_now,"vol_avg":vol_avg,"vol_ratio":vol_ratio,"vol_trend":vol_trend,"pre_price":round(pre_price,2) if pre_price else None,"pre_change":round(pre_change*100,2) if pre_change and abs(pre_change)<1 else round(pre_change,2) if pre_change else None,"post_price":round(post_price,2) if post_price else None,"post_change":round(post_change*100,2) if post_change and abs(post_change)<1 else round(post_change,2) if post_change else None,"short":round(si,1),"price_trend":pt,"ok":True}
    except:
        return {"price":0,"change":0,"cap":0,"cap_str":"N/D","vol_now":0,"vol_avg":0,"vol_ratio":0,"vol_trend":"N/D","pre_price":None,"pre_change":None,"post_price":None,"post_change":None,"short":0,"price_trend":"N/D","ok":False}

@st.cache_data(ttl=3600)
def get_trials(company):
    try:
        r=requests.get("https://clinicaltrials.gov/api/v2/studies",params={"query.spons":company,"filter.overallStatus":"RECRUITING|ACTIVE_NOT_RECRUITING|ENROLLING_BY_INVITATION|NOT_YET_RECRUITING","pageSize":8,"fields":"NCTId,BriefTitle,Phase,Condition,EnrollmentInfo,DesignInfo,PrimaryOutcome,PrimaryCompletionDate"},timeout=8)
        if r.status_code!=200: return []
        out=[]
        for s in r.json().get("studies",[]):
            p=s.get("protocolSection",{});ident=p.get("identificationModule",{});stat=p.get("statusModule",{});des=p.get("designModule",{});cond=p.get("conditionsModule",{});outc=p.get("outcomesModule",{})
            phases=des.get("phases",[]) if des else [];ei=des.get("enrollmentInfo",{}) if des else {};di=des.get("designInfo",{}) if des else {};mi=di.get("maskingInfo",{}) if di else {};po=outc.get("primaryOutcomes",[]) if outc else [];pcd=stat.get("primaryCompletionDateStruct",{}) if stat else {}
            out.append({"nct":ident.get("nctId",""),"title":ident.get("briefTitle",""),"phase":", ".join(phases) if phases else "N/A","conditions":", ".join(cond.get("conditions",[])[:2]) if cond else "N/D","enrollment":ei.get("count",0) if ei else 0,"mask":mi.get("masking","") if mi else "","is_rand":"RANDOMIZED" in (di.get("allocation","").upper()) if di else False,"endpoint":po[0].get("measure","") if po else "","completion":pcd.get("date","") if pcd else ""})
        return out
    except: return []
...
def calc_window(cs,ps):
    ...
    ws=cd+timedelta(weeks=2);we=cd+timedelta(weeks=8);p=ps.upper().replace(" ","")
    if "PHASE3" in p: pkey="3"
    elif "PHASE2" in p: pkey="2"
    elif "PHASE1" in p: pkey="1"
    else: return None
    rup={"3":45,"2":30,"1":15}[pkey];rs=ws-timedelta(days=rup);now=datetime.now()
...
def render_metrics(stk):
    ...
    m1.metric("Prezzo",f"${stk['price']}",f"{stk['change']:+.1f}%",delta_color="normal" if stk["change"]>=0 else "inverse")
    m2.metric("Market Cap",stk["cap_str"])
    m3.metric("Vol/Media",f"{stk['vol_ratio']}x" if stk["vol_ratio"] else "N/D","ESPLOSIVO" if stk["vol_ratio"]>=3 else "Alto" if stk["vol_ratio"]>=2 else None)
    m4.metric("Short",f"{stk['short']}%","Molto alto" if stk["short"]>=20 else "Elevato" if stk["short"]>=10 else None)
    ...
    pm1.metric("Pre-Market",f"${stk['pre_price']}" if stk.get("pre_price") else "Chiuso",f"{stk['pre_change']:+.1f}%" if stk.get("pre_change") else None)
    pm2.metric("After-Hours",f"${stk['post_price']}" if stk.get("post_price") else "Chiuso",f"{stk['post_change']:+.1f}%" if stk.get("post_change") else None)

def render_trials(trials, ticker):
    with st.expander(f"Trial ({len(trials)}) — {ticker}"):
        if not trials: st.write("Nessun trial trovato."); return
        for t in trials:
            w=calc_window(t["completion"],t["phase"]);ep=ep_it(t["endpoint"]);ph=t["phase"].upper().replace(" ","")
...
for pm in PDUFA_LIST:
    try: g=(datetime.strptime(pm["pdufa"],"%Y-%m-%d").date()-datetime.now().date()).days
    except: g=-999
    if g<-5: continue
    if g<=0: st.error(f"VERDETTO OGGI: **{pm['ticker']}** — {pm['drug']} ({pm['indication']})")
    elif g<=3: st.error(f"VERDETTO TRA {g}g: **{pm['ticker']}** — {pm['drug']} ({pm['indication']})")
    elif g<=7: st.warning(f"VERDETTO TRA {g}g: **{pm['ticker']}** — {pm['drug']} ({pm['indication']})")
    else: st.info(f"VERDETTO TRA {g}g: **{pm['ticker']}** — {pm['drug']} ({pm['indication']})")
    st.write(pm["nota"])
    ...
    stk=get_stock(pm["ticker"])
    if stk["ok"]: render_metrics(stk)
    else: st.warning(f"Dati mercato non disponibili per {pm['ticker']}")
...
if radar_hot:
    for item in radar_hot:
        stk=item["stk"];alert=item["alert"]
        badge="ALTO" if alert>=7 else "MEDIO" if alert>=4 else "BASSO"
...
with st.expander("Legenda + limiti"):
    st.write("Volume/media = volume oggi / media 3 mesi")
    st.write("Pre-market 4:00-9:30 ET | After-hours 16:00-20:00 ET")
    st.write("Limiti: yfinance non ufficiale | Volume non distingue buy/sell | Aggiorna PDUFA settimanalmente")

st.caption(f"v13 | PDUFA: {DATA_AGG} | App educativa — non e consulenza finanziaria | fda.gov")
