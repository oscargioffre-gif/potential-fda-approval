# FDA RADAR v12 — Volumi + Storico + ALDX fix
...
STORICO = [
    {"data":"2026-03-11","ticker":"WELL","azienda":"Wellcovorin","farmaco":"Leucovorin calcium",
     "esito":"APPROVATO","fase":"sNDA","nota":"Cerebral folate deficiency.","move":"+12%"},
    {"data":"2026-03-06","ticker":"LNTH","azienda":"Lantheus","farmaco":"PYLARIFY TruVu",
     "esito":"APPROVATO","fase":"NDA","nota":"Imaging prostatico.","move":"+8%"},
    {"data":"2026-03-06","ticker":"BMY","azienda":"Bristol Myers Squibb","farmaco":"Sotyktu",
     "esito":"APPROVATO","fase":"sNDA","nota":"Artrite psoriasica.","move":"+3%"},
    {"data":"2026-02-25","ticker":"ENSG","azienda":"Ensergo","farmaco":"DESMODA",
    ...
    {"data":"2026-01-14","ticker":"CUTX","azienda":"Sentynl/Cyprium","farmaco":"CUTX-101",
     "esito":"APPROVATO","fase":"NDA","nota":"Malattia di Menkes.","move":"+85%"},
    {"data":"2026-01-10","ticker":"ATRA","azienda":"Atara Bio","farmaco":"Tabelecleucel",
     "esito":"BOCCIATO","fase":"BLA","nota":"EBV+ PTLD. CRL.","move":"-62%"},
    {"data":"2025-12-16","ticker":"ALDX","azienda":"Aldeyra","farmaco":"Reproxalap",
     "esito":"RINVIATO","fase":"NDA","nota":"PDUFA esteso a mar.2026.","move":"+18%"},
]
...
def compute_alert(stk, window, pdufa_match):
    ...
    if pdufa_match:
        try:
            ...
            if -2 <= g <= 0:
                signals.append("VERDETTO FDA OGGI/IMMINENTE"); alert+=5
            elif 0 < g <= 3:
                signals.append(f"VERDETTO FDA TRA {g}g"); alert+=4
            elif 3 < g <= 14:
                signals.append(f"Verdetto FDA tra {g}g"); alert+=3
        except: pass
        if pdufa_match.get("labeling"): signals.append("FDA scrive il foglietto"); alert+=2
...
with st.expander("Come leggere i segnali"):
    ...
    st.write("Volume/media 2x = attivita sopra la norma")
    st.write("Pre-market/After-hours = movimenti fuori orario, spesso su notizie")
...
for item in radar_hot:
    ...
    elif stk["vol_trend"]=="crescente": v3.metric("Trend 5gg","Crescente")
    elif stk["vol_trend"]=="calante": v3.metric("Trend 5gg","Calante")
...
st.caption("Approvati, bocciati e rinviati con movimento titolo")

for item in STORICO:
    if item["esito"] == "APPROVATO": icon = "APPROVATO"
    elif item["esito"] == "BOCCIATO": icon = "BOCCIATO"
    elif item["esito"] == "RINVIATO": icon = "RINVIATO"
    else: icon = item["esito"]
    st.write(f"**{item['data']}** — **{item['ticker']}** ({item['azienda']}) — {item['farmaco']}")
    st.caption(f"{icon} | {item['fase']} | Movimento: **{item.get('move','N/D')}** | {item['nota']}")
    st.divider()

# FOOTER
with st.expander("Legenda + limiti"):
    st.write("Volume/media = volume oggi / media 3 mesi. 2x+ = insolito.")
    st.write("Pre-market = scambi prima apertura (4:00-9:30 ET).")
    st.write("After-hours = scambi dopo chiusura (16:00-20:00 ET).")
    st.write("Short % = % azioni scommesse contro.")
    st.write("Limiti: yfinance non ufficiale | Volume non distingue buy/sell | Pre/post solo durante sessioni estese | Storico hardcoded | Aggiorna PDUFA settimanalmente")

st.caption(f"v12 | PDUFA: {DATA_AGG} | App educativa — non e consulenza finanziaria | fda.gov")
